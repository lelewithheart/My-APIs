...
class ApiSpecValidationTest(unittest.TestCase):
    def test_all_api_folders_have_openapi_spec(self) -> None:
        ...
        self.assertGreaterEqual(len(api_folders), 18, "expected an expanded API catalog")
        for folder in api_folders:
            ...
            self.assertTrue(docs.exists(), f"{folder.name} is missing README.md documentation")
            content = spec.read_text(encoding="utf-8")
            doc_content = docs.read_text(encoding="utf-8")
